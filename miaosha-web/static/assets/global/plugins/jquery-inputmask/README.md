...
Include the js-files which you can find in the dist-folder. You have the bundled file which contains the main plugin
code and also all extensions (date, numerics, other) or if you prefer to only include some parts, use the separate
js-files in the dist/min folder.
...
Any option can also be passed through the use of a data attribute. Use data-inputmask-***the name op the option***="
value"
...
overridden
by giving a definitionSymbol. (see example x, y, z, which can be used for ip-address masking, the validation is
different, but it is allowed to shift the characters between the definitions)
...
Executes after unmasking to allow post-processing of the unmaskedvalue. The arguments to the function are maskedValue,
unmaskedValue.
...
When both the required and optional parts of the mask are filled out and the field loses focus, the user will
see `123-ABC`.
...
masks or retrieving mask definitions dynamically through ajax.
...
This callback allows for preprocessing the initial value before actually handling the value for masking. This can be
usefull for stripping away some characters before processing.
...
There is autocompletion on tab with decimal numbers. You can disable this behaviour by setting the skipRadixDance to
true.
